...
const LaidBarChartComponent = ({ data }: { data: any }) => {
    ...
    function extractValuesAndDates() {
        const values: number[] = jsonData.map((entry: { value: number }) => Math.abs(entry.value * 1000));
        const dates: string[] = jsonData.map((entry: { date: string }) => entry.date);
        setChartData({ values, dates });
    }
        ...
        <div className='bg-white p-[24px] h-[350px] w-full rounded-[7px] border border-[#DFE6E9]'>
            ...
            <div className='flex flex-row justify-between items-center mb-[16px]'>
                <div className='flex items-baseline'>
                    <span className='font-medium text-[16px] leading-[24px] text-[#2D3436]'>Bar Chart</span>
...
export default LaidBarChartComponent;